...
#if defined(CONFIG_MTD_NAND_OMAP_BCH)
#include <linux/bch.h>
#define GPMC_ECC_BCH_RESULT_0   0x240
#define DEBUG_ECC
#if defined(CONFIG_MTD_NAND_OMAP_BCH_8)
#define BCH_ERROR_CAP           8
#define ECC_REPORT_THRESHOLD    3
#elif defined(CONFIG_MTD_NAND_OMAP_BCH_4)
#define BCH_ERROR_CAP           4
#define ECC_REPORT_THRESHOLD    2
#endif
#endif
...
struct nand_ecclayout *get_ecc_layout(int bus_width)
{
	...
	/* binary bch code */
	nand_oob.eccbytes = 4*((BCH_ERROR_CAP == 8)? 13 : 7);
#else
	/* hamming code */
				...
				u_char *read_ecc, u_char *calc_ecc)
{
	...
	eccsize = (BCH_ERROR_CAP == 8)? 13 : 7;

	for (i = 0; i < blockCnt; i++) {

		/* compare read and calculated ecc */
		for (j = 0, eccflag = 0; j < eccsize; j++) {
			calc_ecc[j] ^= read_ecc[j];
			eccflag |= calc_ecc[j];
		}
		if (eccflag) {
			count = decode_bch(info->bch, NULL, 512, NULL, calc_ecc,
					   NULL, errloc);
			if (count > 0) {
				printk(KERN_INFO "ecc correction in bits ");
				/* correct errors */
				for (j = 0; j < count; j++) {
				...
				/*
				 * HACK - HACK - HACK
				 * we want to prevent UBI from torturing and
				 * marking a block as bad as soon as 1 bitflip
				 * is persistent; hence, we implement a
				 * threshold below which we do not report
				 * errors yet.
				 */
				if (count < ECC_REPORT_THRESHOLD)
					count = 0;
				else
					printk(KERN_INFO
					       "reporting ecc correction\n");
				if (ret >= 0) {
					ret += count;
				}
			}
			...
			if (count < 0) {
				ret = -1;
				printk(KERN_ERR "ecc failure !\n");
#if defined(DEBUG_ECC)
				printk("calc_ecc[%d] = {", eccsize);
				for (j = 0; j < eccsize; j++)
					printk("%x,", calc_ecc[j]);
				printk("}\nread_ecc[%d] = {", eccsize);
				for (j = 0; j < eccsize; j++)
...
static void omap_enable_hwecc(struct mtd_info *mtd, int mode)
{
	...
	struct nand_chip *chip = mtd->priv;
	unsigned int dev_width = (chip->options & NAND_BUSWIDTH_16)? 1 : 0;
	unsigned long ecc_ctl = 0, val;

	/* is there any side-effect to this ? */
	val = __raw_readl(info->gpmc_baseaddr + GPMC_ECC_CONFIG);
	...
	default:
		DEBUG(MTD_DEBUG_LEVEL0, "error: unknown mode %d!\n", mode);
		break;
	}

	__raw_writel(ecc_ctl, info->gpmc_baseaddr + GPMC_ECC_CONTROL);
	/* (ECC 16 or 8 bit col) | ( CS  )  | ECC Enable */
	val = (dev_width << 7) | (info->gpmc_cs << 1) | 1;
...
#ifdef CONFIG_MTD_NAND_OMAP_BCH
		/* configure software bch library, only used for decoding */
		info->bch = init_bch(13, BCH_ERROR_CAP, 0x201b);
		if (!info->bch)
			goto out_release_mem_region;
		info->nand.ecc.bytes            = (BCH_ERROR_CAP == 8)? 52 : 28;
		info->nand.ecc.size             = 2048;
#else
		/* Hamming */
