...
static struct omap_sdrc_params parrot_sdrc_params[2];
...
void __init parrot_omap_init_irq(int rate)
{
	 ...
	 */
	parrot_sdrc_params[0].rate = rate * 1000000;

	parrot_sdrc_params[0].actim_ctrla = sdrc_read_reg(SDRC_ACTIM_CTRL_A_0);
	parrot_sdrc_params[0].actim_ctrlb = sdrc_read_reg(SDRC_ACTIM_CTRL_B_0);
	parrot_sdrc_params[0].rfr_ctrl = sdrc_read_reg(SDRC_RFR_CTRL_0);
	parrot_sdrc_params[0].mr = sdrc_read_reg(SDRC_MR_0);

	/* if SDRC_RFR_CTRL_1 = 0 (reset value), CS1 is not used */
	parrot_sdrc_params[1].rate = rate * 1000000;

	parrot_sdrc_params[1].actim_ctrla = sdrc_read_reg(SDRC_ACTIM_CTRL_A_1);
	parrot_sdrc_params[1].actim_ctrlb = sdrc_read_reg(SDRC_ACTIM_CTRL_B_1);
	parrot_sdrc_params[1].rfr_ctrl = sdrc_read_reg(SDRC_RFR_CTRL_1);
	parrot_sdrc_params[1].mr = sdrc_read_reg(SDRC_MR_1);

	omap2_init_common_hw(
			&parrot_sdrc_params[0], &parrot_sdrc_params[1],
